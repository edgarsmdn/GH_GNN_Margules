...
class GH_GNN_old():
    def __init__(self, solute_smiles, solvent_smiles):
        ...
        path_parameters = current_path+'/organic_old_GHGNN.pth'
    ...
    def check_interpolation(self, solvent, solute):
        ...
        with open(current_path+'/training_solutes_organic.pickle', 'rb') as handle:
            training_solutes = pickle.load(handle)
            
        with open(current_path+'/training_solvents_organic.pickle', 'rb') as handle:
    ...
    def indicator_class(self, solvent, solute):
        ...
        with open(current_path+'/training_classes_organic.pickle', 'rb') as handle:
    ...
    def indicator_tanimoto(self, solvent, solute):
        ...
        with open(current_path+'/training_fp_organic.pickle', 'rb') as handle:
    ...
    def predict(self, T, constants=False):
        ...
        T = T - 273.15 # old version was trained on Celsius
        ...
        with torch.no_grad():
            for batch_solvent, batch_solute, batch_T in predict_loader:
                ...
                if torch.cuda.is_available():
                    pred  = self.model(batch_solvent.cuda(), batch_solute.cuda(), batch_T.cuda(), constants=constants, old=True)
                    K1, K2 = pred
                    K1, K2 = K1.cpu(), K2.cpu()
                else:
