...
from models.GHGNN.GHGNN_architecture import GHGNN_model
import torch
...
possible_formal_charge = [0,1,-1,2,-2,3,-3]
...
class GH_GNN():
    def __init__(self, solute_smiles, solvent_smiles, version='combined'):
        self.version_AD = version
        ...
        if version == 'combined':
            path_parameters = current_path+'/combined_GHGNN.pth'
        elif version == 'organic':
            path_parameters = current_path+'/organic_GHGNN.pth'
    ...
    def check_interpolation(self, solvent, solute):
        ...
        with open(current_path+f'/training_solutes_{self.version_AD}.pickle', 'rb') as handle:
            training_solutes = pickle.load(handle)
            
        with open(current_path+f'/training_solvents_{self.version_AD}.pickle', 'rb') as handle:
    ...
    def indicator_class(self, solvent, solute):
        ...
        with open(current_path+f'/training_classes_{self.version_AD}.pickle', 'rb') as handle:
    ...
    def indicator_tanimoto(self, solvent, solute):
        ...
        with open(current_path+f'/training_fp_{self.version_AD}.pickle', 'rb') as handle:
        ...
        with torch.no_grad():
            for batch_solvent, batch_solute, batch_T in predict_loader:
                ...
                if torch.cuda.is_available():
                    pred  = self.model(batch_solvent.cuda(), batch_solute.cuda(), batch_T.cuda(), constants=constants)
                    K1, K2 = pred
                    K1, K2 = K1.cpu(), K2.cpu()
                else:
