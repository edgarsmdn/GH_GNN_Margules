...
from models.GHGNN.mol2graph import cat_dummy_graph
from models.GHGNN.mol2graph_old import cat_dummy_graph_old
...
class GHGNN_model(nn.Module):
    ...
    def forward(self, solvent, solute, T, constants=False, old=False):
        ...
        if solvent.edge_attr.shape[0] == 0 or solute.edge_attr.shape[0] == 0:
            if old:
                solvent = cat_dummy_graph_old(solvent)
                solute = cat_dummy_graph_old(solute)
            else:
        ...
        if constants:
            ...
            T = T.x.reshape(-1,1) + 273.15 # This seems like a small typo, but the original GH-GNN was trained on this, so the weights work for this
            output = A + B/T
            return output
